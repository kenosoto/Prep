...
function arrayContiene(array, elemento) {
  ...
  // Tu código:
  // Solución A: "return array.includes(elemento);"
  for (var i = 0; i < array.length; i++) {
    if (array[i] === elemento) {
      return true;
    }
...
function numeroMasGrande(numeros) {
  ...
  // Tu código:
  var maxGrande = numeros[];
  for (var i = 1; i < numeros.length; i++) {
    if (numeros[i] > maxGrande) {
      maxGrande = numeros[i];
    }
  }
  return maxGrande;
...
function mesesDelAño(array) {
  ...
  if (nuevosMeses.length < 3) return "No se encontraron los meses pedidos";
  return nuevosMeses;
}
